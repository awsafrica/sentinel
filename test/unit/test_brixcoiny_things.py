...
def test_valid_dash_address():
    ...
    test = valid_brixcoin_address('testnet')

    assert is_valid_brixcoin_address(main) is True
    assert is_valid_brixcoin_address(main, 'mainnet') is True
    assert is_valid_brixcoin_address(main, 'testnet') is False

    assert is_valid_brixcoin_address(test) is False
    assert is_valid_brixcoin_address(test, 'mainnet') is False
...
def test_invalid_dash_address():
    ...
    assert is_valid_brixcoin_address(main) is False
    assert is_valid_brixcoin_address(main, 'mainnet') is False
    assert is_valid_brixcoin_address(main, 'testnet') is False

    assert is_valid_brixcoin_address(test) is False
    assert is_valid_brixcoin_address(test, 'mainnet') is False
    assert is_valid_brixcoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from brixcoinlib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % brixcoinlib.hashit(sb_data_hex)
