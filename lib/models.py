...
class GovernanceObject(BaseModel):
    ...
    def import_gobject_from_brixcoind(self, brixcoind, rec):
        ...
        # exclude any invalid model data from brixcoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from brixcoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from brixcoind - Brixcoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(brixcoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from brixcoind! %s" % e)
            govobj.vote_delete(brixcoind)
            return (govobj, None)
    ...
    def vote_delete(self, brixcoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(brixcoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, brixcoind, signal, outcome):
        import brixcoinlib
        ...
        output = brixcoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = brixcoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(brixcoind, signal)

    def sync_network_vote(self, brixcoind, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = brixcoind.get_my_gobject_votes(self.object_hash)
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BRIXCOIND_GOVOBJ_TYPES['superblock']
